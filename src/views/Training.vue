  ...
  <v-container fluid class="h-100 d-flex flex-column">
    ...
    <div class="mb-4" v-if="canSeeGuides">
      ...
      <v-row>
        <v-col cols="12" sm="6" md="3" v-for="guide in visibleGuides" :key="guide.id">
          <v-card @click="openGuide(guide)" hover variant="outlined" class="h-100 d-flex flex-column" :color="guide.id === 'grenouille' ? 'green-darken-4' : (guide.id === 'conduite' ? 'orange-darken-2' : getGuideTheme(guide.id))">
    ...
    <v-dialog v-model="scenarioDialog" max-width="600px" scrollable>
      <v-card>
        ...
        <v-card-text class="pa-4" style="max-height: 70vh;">
          <v-autocomplete v-model="selectedScenarioTrainee" :items="trainees" item-title="name" return-object label="Choisir un interne ou résident" variant="outlined" class="mb-4" @update:model-value="resetScenarioSelection"></v-autocomplete>

          <div v-if="selectedScenarioTrainee">
            <v-select v-model="selectedScenarioObjectives" :items="availableObjectives" item-title="title" return-object label="Objectifs à travailler" variant="outlined" class="mb-4" multiple chips @update:model-value="selectedScenarios = []">
            ...
            <div class="d-flex justify-space-between align-center mb-4">
              <v-btn color="purple-lighten-2" variant="tonal" prepend-icon="mdi-shuffle" @click="pickRandomSimulation" :disabled="availableObjectives.filter(o => o.current < o.target).length === 0">
                Aléatoire (Non acquis)
              </v-btn>
              <v-btn color="grey" variant="text" prepend-icon="mdi-close" @click="resetScenarioSelection" v-if="selectedScenarioObjectives.length > 0">
                Tout désélectionner
              </v-btn>
            </div>
            ...
            <div v-if="generatedInjuries.length > 0">
              ...
              <div class="d-flex justify-center flex-wrap gap-4 mt-3">
                <v-btn color="blue-grey" variant="tonal" prepend-icon="mdi-pin" density="compact" class="mr-2" @click="pinSimulation(false)">
                  Entraînement
                </v-btn>
                <v-btn color="success" variant="tonal" prepend-icon="mdi-pin" density="compact" @click="pinSimulation(true)">
                  Validante
                </v-btn>
              </div>
            </div>
    ...
    <v-dialog v-model="addTrainingDialog" max-width="400px">
      <v-card v-if="selectedTrainingEmployee">
        ...
        <v-card-text class="pt-4">
          <v-select v-model="newValidatedTraining" :items="availableTrainingsToAdd" label="Formation" variant="outlined"></v-select>
        </v-card-text>
        <v-card-actions>
          <v-spacer></v-spacer>
                    ...
                    <tr v-for="(step, j) in group.items" :key="step.originalIndex" class="transition-swing hover:bg-grey-lighten-5">
                      ...
                      <td class="pt-3 pb-3 align-top text-body-2 text-high-emphasis" :style="j === group.items.length - 1 ? 'border-bottom: 2px solid #9e9e9e !important' : ''">
                        ...
                        <div v-if="step.image" class="mt-2">
                          <v-img :src="step.image" max-height="600" max-width="100%" class="rounded elevation-2 cursor-pointer" cover @click="openImageZoom(step.image)">
                      ...
                      <td class="align-top pt-2 text-center" :style="j === group.items.length - 1 ? 'border-bottom: 2px solid #9e9e9e !important' : ''">
                        <v-checkbox-btn v-model="checkedSteps[step.originalIndex]" :color="getGuideTheme(currentGuide.id)" density="compact" hide-details></v-checkbox-btn>
                      </td>
                    </tr>
                  </template>
                ...
                <v-card-text class="pa-3">
                  <v-row no-gutters align="start">
                    ...
                    <v-col cols="11">
                      <v-row dense>
                        <v-col cols="12" sm="6">
                          <v-text-field v-model="step.header" label="Catégorie (Header)" variant="outlined" density="compact" hide-details="auto" class="mb-2"></v-text-field>
                        </v-col>
                        <v-col cols="12" sm="6">
                          <v-text-field v-model="step.title" label="Titre" variant="outlined" density="compact" hide-details="auto" class="mb-2"></v-text-field>
                        </v-col>
                        <v-col cols="12">
                          <v-textarea v-model="step.description" label="Description" variant="outlined" density="compact" hide-details="auto" rows="2" auto-grow></v-textarea>
                          <div class="d-flex align-center gap-2 mt-2">
                            <v-text-field v-model="step.image" label="URL Image" variant="outlined" density="compact" hide-details="auto" class="flex-grow-1" prepend-inner-icon="mdi-image"></v-text-field>
                            <v-btn class="ml-2" color="orange-darken-2" variant="tonal" size="small" prepend-icon="mdi-upload" :loading="uploadingStepIndex === i" @click="$refs['imgUpload_' + i][0].click()">
                              Upload
                            </v-btn>
                            <input :ref="'imgUpload_' + i" type="file" accept="image/*" style="display:none" @change="uploadStepImage(i, $event)" />
                          </div>
                        </v-col>
                      </v-row>
                    </v-col>
              ...
              <v-col cols="12" md="6" v-for="comp in category.competencies" :key="comp.id">
                <v-card variant="outlined" class="h-100">
                  <v-card-title class="text-subtitle-2 font-weight-bold d-flex align-center">
                    {{ comp.title }}
                    <v-spacer></v-spacer>
                    <v-btn icon density="compact" variant="text" size="small" class="mr-2" @click.stop="toggleCompetencySeen(comp.id)">
                    ...
                    <v-progress-circular :model-value="getCompetencyProgress(comp)" :color="getCompetencyColor(getCompetencyProgress(comp))" size="24" width="4"></v-progress-circular>
                  </v-card-title>
                  <v-divider></v-divider>
                  <v-card-text class="pa-0">
                    <v-list density="compact">
                      <v-list-item v-for="sub in comp.subCompetencies" :key="sub.id" @click="toggleSubCompetency(sub.id)">
                        <template v-slot:prepend>
                          <v-checkbox-btn :model-value="isSubCompetencyValidated(sub.id)" color="success" hide-details density="compact"></v-checkbox-btn>
                        </template>
    ...
    <v-dialog v-model="objectivesDialog" fullscreen transition="dialog-bottom-transition">
      <v-card>
        ...
        <v-card-text style="overflow-y: auto;">
          ...
          <v-table density="compact" class="mb-6">
            ...
            <tbody>
              <tr v-for="emp in interns" :key="emp.id">
                ...
                <td v-for="obj in internObjectives" :key="obj.hash" class="text-center cursor-pointer hover-bg" @click="openQuickValidation(emp, obj.id)">
                  <v-icon v-if="calculateProgress(emp, obj.id) >= obj.target" color="success">mdi-check-circle</v-icon>
                  <v-progress-linear v-else :model-value="calculateProgress(emp, obj.id)" :color="getCompetencyColor(calculateProgress(emp, obj.id), obj.target)" height="15" rounded>
          ...
          <v-table density="compact">
            ...
            <tbody>
              <tr v-for="emp in residentsList" :key="emp.id">
                ...
                <td v-for="obj in residentObjectives" :key="obj.hash" class="text-center cursor-pointer hover-bg" @click="openQuickValidation(emp, obj.id)">
                  <v-icon v-if="calculateProgress(emp, obj.id) >= obj.target" color="success">mdi-check-circle</v-icon>
                  <v-progress-linear v-else :model-value="calculateProgress(emp, obj.id)" :color="getCompetencyColor(calculateProgress(emp, obj.id), obj.target)" height="15" rounded>
                    <template v-slot:default="{ value }">
                      <span class="text-caption text-white">{{ Math.ceil(value) }}%</span>
                  ...
                  <td v-for="obj in trainerObjectives" :key="obj.id" class="text-center" style="min-width: 120px;">
                    <v-menu>
                      <template v-slot:activator="{ props }">
                        <v-chip v-bind="props" :color="getTrainerStatusColor(getTrainerStatus(emp, obj.id))" size="small" variant="flat" class="cursor-pointer">
                          {{ getTrainerStatusLabel(getTrainerStatus(emp, obj.id)) }}
                        </v-chip>
                      </template>
                      <v-list density="compact">
                        <v-list-item v-for="status in trainerStatuses" :key="status.value" @click="setTrainerStatus(emp, obj.id, status.value)">
                          <template v-slot:prepend>
          ...
          <v-autocomplete v-model="selectedTrainerTrainee" :items="availableTrainerTrainees" item-title="name" return-object label="Sélectionner un Formateur" variant="outlined" density="compact" clearable :custom-filter="customFilter" class="mb-2" style="max-width: 400px;">
            <template v-slot:append>
    ...
    <v-dialog v-model="subCompetenciesDialog" max-width="500" z-index="2500">
      <v-card v-if="currentTrainee">
        ...
        <v-card-text class="pa-0">
          <v-list density="compact">
            <v-list-item v-for="sub in currentSubCompetencies" :key="sub.id" @click="toggleSubVal(sub.id)" active-color="primary">
              <template v-slot:prepend>
    ...
    <v-dialog v-model="allValidationsDialog" max-width="1000px" scrollable>
      <v-card>
        ...
        <v-card-text class="pa-4" style="max-height: 70vh;">
          <v-text-field v-model="allValidationsSearch" prepend-inner-icon="mdi-magnify" label="Rechercher un membre" variant="outlined" density="compact" class="mb-4" hide-details></v-text-field>
          <v-data-table :headers="allValidationsHeaders" :items="employees" :search="allValidationsSearch" :sort-by="[{ key: 'role', order: 'asc' }]" :items-per-page="40" density="compact">
    ...
    <v-card class="flex-grow-1" v-if="!isRestrictedTrainer">
      <v-data-table :headers="headers" :items="trainees" :search="search" class="h-100" :sort-by="[{ key: 'role', order: 'desc' }]" :items-per-page="25">
        ...
        <template v-slot:item.badges="{ item }">
          <div class="d-flex align-center justify-center gap-1">
            <span v-for="(badge, i) in getEmployeeBadges(item)" :key="i" class="text-h6 cursor-help cursor-pointer">
              {{ badge.emoji }}
              <v-tooltip activator="parent" location="top">
                {{ badge.title }}
              </v-tooltip>
        ...
        <template v-slot:item.actions="{ item }">
          <v-btn v-if="['Interne', 'Résident', 'Titulaire'].includes(item.role)" color="info" size="small" variant="tonal" prepend-icon="mdi-school" @click="openCompetencyTracking(item)">
            Suivi
          </v-btn>
        </template>
...
export default {
  ...
  methods: {
    ...
    async uploadStepImage(stepIndex, event) {
      ...
      try {
        ...
        const fileRef = storageRef(storage, `training/${filename}`)
        await uploadBytes(fileRef, file)
        const url = await getDownloadURL(fileRef)
        this.editedGuide.steps[stepIndex].image = url
      } catch (e) {
        console.error(e)
