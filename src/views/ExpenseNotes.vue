  ...
  <div style="height: calc(100% - 42px);">
    <v-card class="ma-5 pa-5 rounded-xl h-100">
      ...
      <v-tabs v-model="tab" class="mt-5" background-color="#0f0e22" color="primary" slider-color="primary" align-tabs="center" justify-center>
        ...
        <v-tab value="waiting" v-if="['Direction','Admin'].includes(this.userStore.profile.role)">
          En attente
        </v-tab>
        <v-tab value="history" v-if="['Direction','Admin'].includes(this.userStore.profile.role)">
          Historique
        </v-tab>
      </v-tabs>
      ...
      <v-tabs-window v-model="tab">
        ...
        <v-tabs-window-item value="waiting" v-if="['Direction','Admin'].includes(this.userStore.profile.role)">
          <WaitingTab />
        </v-tabs-window-item>
