<template>
  <div>
    ...
    <v-data-table :headers="headers" :items="vehicles" items-per-page="-1" no-data-text="Aucun vehicle" class="mt-5">
...
export default {
  ...
  methods: {
    ...
    async deleteVehicle(vehicle) {
      const result = await Swal.fire({
        title: 'Êtes-vous sûr ?',
        text: `Voulez-vous vraiment supprimer le véhicule "${vehicle.name}" ?`,
        icon: 'warning',
        showCancelButton: true,
        confirmButtonText: 'Oui',
        cancelButtonText: 'Annuler',
